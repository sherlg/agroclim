...
#' 
#' @description Calculates the number of days where the daily minimum temperature is above than a threshold within a predefined period.
...
#' 
#' @return A numeric vector with the number of frost-free days (days where minimum temperature is below the threshold) per season is returned.
#' 
...
hn <- function(mn, dates, iniday = '07-01', endday = '06-30', thres = 0) {
  ...
  for(i in seq_along(start_idx)) {
    ...
    # Identify days with min temp > thres
